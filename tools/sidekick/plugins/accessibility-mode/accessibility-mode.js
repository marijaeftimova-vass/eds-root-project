...
const createDialog = () => {
  const dialog = createElement('div', { id: 'hlx-a11y-mode-dialog' }, [
    createElement('div', { class: 'hlx-a11y-mode-dialog-container' }, [
      createElement(
        'h4',
        { class: 'hlx-a11y-mode-dialog-title' },
        'Welcome to Accessibility Mode!',
      ),
      createElement(
        'p',
        {},
        'Accessibility Mode is a tool that helps you to identify accessibility issues on your page. It scans the page from a content perspective and reports on things like missing alt text, missing headings, etc.',
      ),
      createElement(
        'p',
        {},
        'Accessibility Mode is not a replacement for a full accessibility audit and does not guarantee compliance with a certain accessibility standard. It is a tool that helps you to identify and fix issues that can be fixed by content authoring.',
      ),
      createElement(
        'p',
        {},
        'The page outline provides a hierarchical view of the page content. It is a great way to understand the structure of the page and to identify any issues with the page structure.',
      ),
      createElement(
        'p',
        {},
        'The readability score provides an indication of how easy it is to read the content on the pag, and it is based on the Flesch Reading Ease test.',
      ),
      createElement(
        'p',
        {},
        'Custom checks for EDS blocks can be added on a per project basis.',
      ),
      createElement(
        'sup',
        {},
        'Note: This tool is extended from the open source project <a href="https://sa11y.netlify.app/overview/" target="_blank">Sa11y</a>.',
      ),
      createElement('div', { class: 'hlx-a11y-mode-dialog-actions' }, [
        createElement(
          'button',
          { class: 'hlx-a11y-mode-dialog-button' },
          'Don’t Show Again',
        ),
      ]),
    ]),
...
const initAccessibilityMode = async (shouldActivateA11yMode) => {
  ...
  if (shouldActivateA11yMode) {
    await loadCSS(
      `${window.hlx.codeBasePath}/tools/sidekick/plugins/accessibility-mode/accessibility-mode.css`,
    );
    ...
    await loadCSS(
      `${window.hlx.codeBasePath}/tools/sidekick/plugins/accessibility-mode/lib/sa11y.min.css`,
    );
